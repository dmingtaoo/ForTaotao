...
func (c *CommandLine) SignatureReturn(args [][]byte) ([]byte,[]byte,error) {
	contractAddr := contract.StringToContractAddress("DID::SPECTRUM::TRADE")
	functionAddr := contract.StringToFunctionAddress("GetDataList")
	tx := eles.Transaction{
		// Sender:    user.Address,
		Sender:    c.defaultAccount.Address,
		Nonce:     0,
		Version:   c.dper.BackStateManager().CurrentVersion(),
		LifeTime:  0,
	...
	tx.SetTxID()
	ID := tx.TxID.Bytes()
	signature, err := c.accountManager.SignHash(c.defaultAccount, tx.TxID)
	if err != nil {
		return signature,ID,err
	}
	tx.Signature = signature

	c.dper.PublishTransactions([]*eles.Transaction{&tx})
	return signature,ID,err
}